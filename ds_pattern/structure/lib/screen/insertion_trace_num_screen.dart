...
class _InsertionTraceNumScreenState extends State<InsertionTraceNumScreen> {
  ...
  void initState() {
    ...
    _eventSubscription = _eventChannel!.receiveBroadcastStream().listen((dynamic event) {
      setState(() {
        _barcodeData = context.read<InsertionTraceNumViewModel>().parsingData(event.toString());
        context.read<InsertionTraceNumViewModel>().traceNum = _barcodeData;
        context.read<InsertionTraceNumViewModel>().textEditingController.text = _barcodeData;
      });
    });
  }
      ...
      child: Scaffold(
        ...
        body: Column(
          children: [
            ...
            if (context.read<InsertionTraceNumViewModel>().isAllInserted == 1)
            ...
            Container(
              ...
              child: MainButton(
                ...
                onPressed: (context.read<InsertionTraceNumViewModel>().isAllInserted == 1)
                    ? () {
                        context.read<InsertionTraceNumViewModel>().saveMeatData();
                        //context.read<InsertionTraceNumViewModel>().asyncPaging();
                      }
                    : null,
              ),
            ),
